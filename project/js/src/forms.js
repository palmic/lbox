...
lbox_forms_show	= function(control) {
	...
	/*if (Fx && Fx.Morph) {
		if (control.sizeOri.y) {
			if (control.getStyle('height').toInt() == 0) {
				control.setStyle('height', 0);
				control.setStyle('display', 'block');
				if (!control.fxmorph) {
					control.fxmorph = new Fx.Morph(control.id, {duration: 'short', transition: Fx.Transitions.Sine.easeOut});
				}
				control.fxmorph.start({'height': [0, control.sizeOri.y]}).chain(
...
lbox_forms_hide	= function(control, calledByChange) {
	...
	/*if (Fx && Fx.Morph) {
		control.sizeOri = control.getSize();
		...
		if (control.getStyle('height').toInt() > 0) {
			...
			control.fxmorph.start({'height': [control.sizeOri.y, 0]});
		}
		else {
